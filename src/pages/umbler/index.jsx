...
import ApiUmbler from '../../services/apiUmbler';
...
const Umbler = () => {
  ...
  useEffect(() => {
    async function fetchClientes() {
      ...
      try {
        const data = await ApiUmbler.getClientes();
        setClientes(data);
      } catch (err) {
        setClientes([]);
      } finally {
  ...
  const handleChatClick = async (cliente) => {
    setLoadingMensagens(true);
    try {
      const mensagens = await ApiUmbler.getMensagensCliente(cliente.id);
      setSelectedClientChat({ ...cliente, mensagens });
              ...
              <div className="fixed inset-0 z-[1300] flex items-center justify-center p-4">
                <div className="bg-card border border-border rounded-lg shadow-xl w-full max-w-md h-[600px] flex flex-col">
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-3">
                    ...
                    ) : selectedClientChat.mensagens.length === 0 ? (
                      <div className="text-center py-8">
                        <Icon name="MessageSquare" size={48} className="text-muted-foreground mx-auto mb-4" />
                        <p className="text-muted-foreground">Nenhuma mensagem encontrada para este cliente.</p>
                      </div>
                    ) : (
